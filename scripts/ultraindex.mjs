...
function runVerify(answerPath, repo, opts = {}) {
  const answer = readFileSync11(answerPath, "utf8");
...
function loadTodoPairs(dir) {
  ...
  let todo;
  try {
    todo = JSON.parse(readFileSync11(p, "utf8"));
  } catch {
    return void 0;
  }
...
function applyVerdicts(dir, verdictsPath) {
  let raw;
  try {
    raw = JSON.parse(readFileSync11(verdictsPath, "utf8"));
  } catch (e) {
...
function listPhases(ctx) {
  ...
  let verifyReason;
  if (existsSync11(verifyWl)) {
    try {
      const todo = JSON.parse(readFileSync12(verifyWl, "utf8"));
