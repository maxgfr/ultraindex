...
// src/cli.ts
import { resolve as resolve10, join as join39, dirname as dirname12 } from "path";
...
function stripFences(content) {
  ...
  for (const line2 of lines) {
    ...
    if (fence2) {
      if (m && line2.trim().startsWith(fence2[0][0].repeat(3).slice(0, 3))) fence2 = null;
      out2.push("");
      continue;
    }
    if (m) {
...
function parseEmbedModel(raw, source) {
  ...
  for (let i2 = 0; i2 < vocabSize; i2++) {
    ...
    const row2 = weights[i2];
    if (!Array.isArray(row2) || row2.length !== dim) {
      throw new Error(`embed model: row ${i2} has length ${row2?.length}, expected ${dim}`);
    }
...
function validateArgs(schema, args2) {
  ...
  for (const [key, value] of Object.entries(args2)) {
    ...
    if (spec.type === "number") {
      ...
      if (spec.minimum !== void 0 && numeric < spec.minimum) return `\`${key}\` must be at least ${spec.minimum}`;
      if (spec.maximum !== void 0 && numeric > spec.maximum) return `\`${key}\` must be at most ${spec.maximum}`;
      continue;
    }
    if (spec.type === "array") {
...
function adjacencyOf(edges, kinds2) {
  ...
  const degree2 = /* @__PURE__ */ new Map();
  for (const e of edges) {
    if (e.dangling) continue;
    if (kinds2 && !kinds2.has(e.kind)) continue;
    (out2.get(e.from) ?? out2.set(e.from, []).get(e.from)).push(e);
    (inn.get(e.to) ?? inn.set(e.to, []).get(e.to)).push(e);
    degree2.set(e.from, (degree2.get(e.from) ?? 0) + 1);
    degree2.set(e.to, (degree2.get(e.to) ?? 0) + 1);
  }
  for (const arr of out2.values()) arr.sort((a, b) => byStr(a.to, b.to));
  for (const arr of inn.values()) arr.sort((a, b) => byStr(a.from, b.from));
  const adj = { out: out2, inn, degree: degree2, threshold: hubThreshold([...degree2.values()]) };
...
function bfs(edges, start2, depth, kinds2) {
  const { out: out2, inn, degree: degree2, threshold } = adjacencyOf(edges, kinds2);
  ...
  for (let d = 1; d <= depth; d++) {
    const next = [];
    for (const node of frontier) {
      if (node !== start2 && (degree2.get(node) ?? 0) >= threshold) continue;
...
// src/build.ts
import { basename as basename4, relative, isAbsolute as isAbsolute2 } from "path";
...
function parseRegions(text) {
  ...
  for (const line2 of lines) {
    const o = OPEN_RE.exec(line2);
    const c2 = CLOSE_RE.exec(line2);
    if (open) {
      if (c2) {
...
function headerRegion(m) {
  ...
  const body2 = [
    ...
    m.summary,
    "",
    `*Module \`${m.slug}\` \xB7 tier ${m.tier} (${TIER_LABEL2[m.tier]}) \xB7 ${m.members.length} files \xB7 ${m.symbols} symbols*`
  ].join("\n");
...
function codeViewRegion(m, records) {
  ...
  for (const rel2 of m.members) {
    const rec = records.get(rel2);
    if (!rec || rec.kind !== "code" || !rec.symbols.length) continue;
    const exported = rec.symbols.filter((s) => s.exported).sort((a, b) => a.line - b.line);
    if (exported.length) apiBlocks.push(renderBlock(rel2, exported));
    else internalBlocks.push(renderBlock(rel2, rec.symbols.slice().sort((a, b) => a.line - b.line)));
  }
  lines.push("");
  if (apiBlocks.length) {
...
function renderIndex(graph, opts) {
  ...
  if (testable.length) {
    ...
    let line2 = `**Tests:** ${testFileCount} test file${testFileCount === 1 ? "" : "s"} \xB7 ${testable.length - untested.length}/${testable.length} code modules tested`;
    if (top.length) {
      const overflow = untested.length > top.length ? ` _(+${untested.length - top.length} more)_` : "";
      line2 += ` \xB7 untested: ${top.map((m) => `\`${m.slug}\``).join(", ")}${overflow}`;
    }
    lines.push("");
    lines.push(line2);
  }
...
function renderMermaid2(graph, opts = {}) {
...
function proseSourceHash(members, hashes) {
  const parts2 = members.slice().sort(byStr).map((rel2) => `${rel2}\0${hashes[rel2] ?? "<deleted>"}`);
...
function buildManifest(scan2, graph, outRel, sync, builtAt, extraNotes = [], filters = {}, prev) {
  ...
  for (const m of graph.modules) {
    ...
    if (digest) {
      ...
      entry2.prose = prevMod.prose;
    }
    modules[m.slug] = entry2;
  }
...
// src/output.ts
import { existsSync as existsSync12, mkdirSync as mkdirSync4, readFileSync as readFileSync16, writeFileSync as writeFileSync5, renameSync as renameSync3, rmSync as rmSync4, readdirSync as readdirSync4 } from "fs";
import { dirname as dirname6, join as join24 } from "path";
function readIfExists(path) {
  try {
    return existsSync12(path) ? readFileSync16(path, "utf8") : void 0;
  } catch {
    return void 0;
  }
}
...
function moveFile(from, to) {
  if (!existsSync12(from)) return;
  mkdirSync4(dirname6(to), { recursive: true });
  renameSync3(from, to);
}
...
function syncEntries(outDir, entries, prevModules) {
  const encDir = join25(outDir, "encyclopedia");
  const orphanDir = join25(encDir, "_orphaned");
  const entryPath = (slug) => join25(encDir, `${slug}.md`);
  ...
  for (const old of goneOld) {
    ...
    if (hasProse2) {
      moveFile(path, join25(orphanDir, `${old}.md`));
      orphaned.push(old);
...
function indexPaths(outDir) {
  return {
    index: join26(outDir, "INDEX.md"),
    graph: join26(outDir, "graph.json"),
    manifest: join26(outDir, "manifest.json"),
    mermaid: join26(outDir, "graph.mmd"),
    encyclopedia: join26(outDir, "encyclopedia"),
    vectors: join26(outDir, "vectors.json"),
    symbols: join26(outDir, "symbols.json"),
    cache: join26(outDir, "cache.json")
  };
}
...
function loadGraph(outDir) {
  ...
  try {
    ...
    return g.schemaVersion === SCHEMA_VERSION2 ? g : void 0;
  } catch {
    return void 0;
  }
}
function loadManifest(outDir) {
  ...
  try {
    ...
    return m.schemaVersion === SCHEMA_VERSION2 ? m : void 0;
  } catch {
    return void 0;
  }
}
function loadSymbols(outDir) {
  ...
  try {
    ...
    return s.schemaVersion === SCHEMA_VERSION2 ? s : void 0;
  } catch {
    return void 0;
  }
}
function loadCache(outDir) {
  ...
  try {
    ...
    if (c2.schemaVersion !== SCHEMA_VERSION2 || c2.extractorVersion !== EXTRACTOR_VERSION) return void 0;
...
// src/find.ts
import { join as join28, basename as basename5, extname as extname2 } from "path";
  ...
  for (const t of terms) {
    ...
    if (exactCount > 0) {
      ...
      for (const f of t.forms) {
        ...
        if (c2 > count2) count2 = c2;
      }
      if (count2 > 0) weight = 2;
      else {
        for (const g of t.groups) {
          const c2 = hay.groups.get(g) ?? 0;
          if (c2 > count2) count2 = c2;
        }
        if (count2 > 0) weight = 1.5;
    ...
    score += (saturate ? weight * Math.min(1.5, 1 + Math.log1p(count2 - 1) * 0.25) : weight) * rarity;
    matched.push(t.raw);
  }
...
function sharedEmbedCacheDir() {
  return join27(dirname7(dirname7(sharedGrammarsCacheDir())), "models");
}
...
function cachedModelDir() {
  ...
  return existsSync13(join27(dir, "model.json")) ? dir : void 0;
}
var QUANT2 = 127;
...
function loadVectors(outDir) {
  ...
  try {
    ...
    return v.schemaVersion === SCHEMA_VERSION2 && v.vectors ? v : void 0;
  } catch {
    return void 0;
  }
}
...
function loadEnrichedProse(outDir, graph) {
  ...
  const out2 = /* @__PURE__ */ new Map();
  for (const m of graph.modules) {
    const text = readIfExists(join28(enc, `${m.slug}.md`));
    if (!text) continue;
...
function scoreModules(graph, query, prose, symbolNames) {
  ...
  const namesOf = (rel2) => (symbolNames?.get(rel2) ?? []).join(" ");
  ...
  for (const m of graph.modules) {
    ...
    const labels = [
      ...
      ...members.map((f) => splitIdentifier(basename5(f.rel, extname2(f.rel))).join(" ")),
...
function expandResults(graph, top, fullScored, seeds, k, enrichedSlugs) {
  ...
  for (let i2 = 0; i2 < queue.length; i2++) {
    ...
    const expand = d < EXPAND_DEPTH && (seedSet.has(slug) || degreeOf2(slug) < threshold);
    if (!expand) continue;
  ...
  const discovered = [...depth.entries()].filter(([, d]) => d >= 1).sort((a, b) => a[1] - b[1] || aff(b[0]) - aff(a[0]) || degreeOf2(b[0]) - degreeOf2(a[0]) || byStr(a[0], b[0]));
...
// src/neighbors.ts
function runNeighbors(outDir, target, depth = 1, kinds2) {
  const graph = loadGraph(outDir);
  if (!graph) return void 0;
  return neighborsOf(graph, target, depth, kinds2);
}
...
function runMap(outDir, moduleSlug) {
  ...
  if (moduleSlug) {
    return readIfExists(join29(paths.encyclopedia, `${moduleSlug}.md`));
  }
  return readIfExists(paths.index);
}

// src/delta.ts
import { join as join30, relative as relative2, isAbsolute as isAbsolute3 } from "path";
...
function runDelta(outDir, repo, opts) {
  ...
  const outRel = relative2(repo, outDir);
  if (!isAbsolute3(outRel) && !outRel.startsWith("..")) {
  ...
  return {
    ...decorated,
    notes: decorated.notes.map(
      (n) => n === "symbol index missing \u2014 symbol-level attribution disabled" ? "symbols.json missing \u2014 symbol-level attribution disabled" : n
    )
  };
}
function formatDeltaPanel2(res) {
  const mb = res.base.mergeBase.slice(0, 7);
  const vs = `${res.base.staged ? "staged vs " : ""}${res.base.ref}`;
  if (!res.changes.length && !res.unindexed.length) {
...
function runStatus(outDir) {
  ...
  const modules = graph.modules.map((m) => {
    ...
    let filled = 0;
    let prose = "none";
    const text = readIfExists(join31(enc, `${m.slug}.md`));
    if (text) {
...
// src/verify.ts
import { existsSync as existsSync14, readFileSync as readFileSync17, readdirSync as readdirSync5, unlinkSync, writeFileSync as writeFileSync6 } from "fs";
import { dirname as dirname8, join as join32, resolve as resolve6 } from "path";
...
function stripInlineCodeText(line2) {
  return line2.replace(/`([^`\n]*)`/g, "$1");
}
...
function isTableSep(line2) {
  return /\|/.test(line2) && /^[\s:|-]+$/.test(line2.trim()) && /-/.test(line2);
}
function isTableRow(line2) {
  return /\|/.test(line2.trim()) && !isTableSep(line2);
}
function tableCells(line2) {
  return line2.trim().replace(/^\|/, "").replace(/\|$/, "").split("|").map((c2) => c2.trim()).join(" ");
}
function isListItem(line2) {
  return /^\s*([-*+]|\d+\.)\s+\S/.test(line2);
}
function extractClaimUnits(text) {
  ...
  while (i2 < lines.length) {
    ...
    const line2 = stripInlineCode(lines[i2]);
    const lineD = stripInlineCodeText(lines[i2]);
    const t = line2.trim();
    if (t === "" || isHeadingOrRule(t) || isTableSep(line2)) {
      flush();
      i2++;
      continue;
    }
    if (isTableRow(line2)) {
      flush();
      units.push({ kind: "text", text: tableCells(line2), display: tableCells(lineD) });
      i2++;
      continue;
    }
    if (/^\s*>/.test(line2)) {
      const dq = line2.replace(/^\s*>\s?/, "").trim();
...
function readExcerpt(repo, c2) {
  let full;
  try {
    full = readFileSync17(join32(repo, c2.path), "utf8");
  } catch {
    return "";
  }
...
function removeStaleBatches(dir, answerPath, current) {
  ...
  for (const entry2 of readdirSync5(dir, { withFileTypes: true })) {
    ...
    const path = join32(dir, entry2.name);
    let batch;
    try {
      batch = JSON.parse(readFileSync17(path, "utf8"));
    } catch {
      continue;
    }
...
function runVerify(answerPath, repo, opts = {}) {
  ...
  if (opts.complete) {
    ...
    for (let i2 = 0; i2 < todo.pairs.length; i2 += size) {
      ...
      writeFileSync6(join32(dir, file), JSON.stringify({ answer: answerPath, coverage, pairs: todo.pairs.slice(i2, i2 + size) }, null, 2));
      worklist.batches.push(file);
    }
  }
  removeStaleBatches(dir, answerPath, worklist.batches ?? []);
  writeFileSync6(join32(dir, "VERIFY.todo.json"), JSON.stringify(todo, null, 2));
  writeFileSync6(join32(dir, "VERIFY.md"), renderWorklistMd(worklist, pairs.length, kept.length));
  return worklist;
}
...
function loadTodoPairs(dir) {
  const p = join32(dir, "VERIFY.todo.json");
  if (!existsSync14(p)) return void 0;
  let todo;
  try {
    todo = JSON.parse(readFileSync17(p, "utf8"));
  } catch {
    return void 0;
  }
...
function applyVerdicts(dir, verdictsPath) {
  ...
  writeFileSync6(join32(dir, "VERIFY.json"), JSON.stringify({ ...result, verdicts }, null, 2));
  return result;
}
...
function loadVerify(dir) {
  const p = join32(dir, "VERIFY.json");
  if (!existsSync14(p)) return void 0;
  try {
...
function runCheck(outDir, repo, opts = {}) {
  ...
  const added = [];
  const removed = [];
  for (const rel2 of Object.keys(current)) {
    if (!(rel2 in recorded)) added.push(rel2);
    else if (current[rel2] !== recorded[rel2]) changed.push(rel2);
  }
  for (const rel2 of Object.keys(recorded)) if (!(rel2 in current)) removed.push(rel2);
  changed.sort(byStr);
  added.sort(byStr);
  removed.sort(byStr);
  const enc = indexPaths(outDir).encyclopedia;
  for (const m of graph.modules) {
    if (readIfExists(join33(enc, `${m.slug}.md`)) === void 0) {
  ...
  for (const m of graph.modules) {
    const text = readIfExists(join33(enc, `${m.slug}.md`));
    if (!text) continue;
    const parsed = parseRegions(text);
    if (!parsed.ok) {
      errors.push(
...
function checkAnswer(outDir, answerPath, opts = {}) {
  ...
  if (manifest && repoRoot && cc.resolved.length) {
    ...
    const drifted = cited.filter((rel2) => {
      const recorded = manifest.fileHashes[rel2];
      return recorded !== void 0 && sha1(readText(join33(repoRoot, rel2))) !== recorded;
    });
    if (drifted.length) {
      warnings.push(
...
// src/evidence.ts
import { join as join34, extname as extname3 } from "path";
var HEAD_LINES = 120;
var MAX_SYMS = 25;
var ASK_FILE_CAP = 20;
function gatherEvidence(repo, rels, headLines = HEAD_LINES) {
  ...
  for (const rel2 of rels) {
    const content = readText(join34(repo, rel2));
    if (!content) continue;
    const lines = content.split(/\r?\n/);
    const code = extractCode(rel2, extname3(rel2).toLowerCase(), content);
...
function fence(rel2) {
  const lang = extToLang(extname3(rel2).toLowerCase());
...
function keyFiles(graph, module2, cap) {
  ...
  return module2.members.filter((rel2) => nodes.get(rel2)?.fileKind === "code").sort((a, b) => {
...
function neighborLines(graph, slug) {
  ...
  const line2 = (s, dir) => `- ${dir} \`${s}\` \u2014 ${clipInline(byId.get(s)?.summary ?? "", 80)}`;
  const side = (ids, dir) => {
    const uniq = [...new Set(ids)].sort(byStr);
    const shown = uniq.slice(0, MAX_NEIGHBORS).map((s) => line2(s, dir));
...
// src/orchestrate.ts
import { existsSync as existsSync15, mkdirSync as mkdirSync5, readFileSync as readFileSync18, writeFileSync as writeFileSync7 } from "fs";
import { dirname as dirname10, join as join36, resolve as resolve7 } from "path";
...
function phaseWorkflowScript(ph, ctx, batchSize) {
  ...
  const scriptPath = join35(ctx.out, "orchestration", `${ph.name}.workflow.mjs`);
...
Index: \`${ctx.out}\` \xB7 Repo: \`${ctx.repo}\`. The queue you were drawn from is exactly what \`${engine} status --out ${ctx.out} --json\` reports: modules whose prose went STALE first (\`prose: "stale"\` \u2014 an explanation written against source that has since changed), then never-enriched ones, most useful first.
...
Return ONLY the structured output specified above. Do NOT write, edit, or delete any file; do NOT run any engine command that writes (\`build\`, \`embed\`, \`verify --apply\`). The orchestrator is the sole writer \u2014 it folds your verdicts into a verdicts file itself and runs the fail-closed \`verify --apply\` gate. Exception: if a justification is prose too large to return, write ONLY to \`${join35(ctx.out, "orchestration", "out")}/<role>-<batch>.md\` (a file namespaced to you alone) and return its path.
`
  };
}
function runbookMd(phases, ctx) {
  ...
  const engine = `node ${ctx.engine}`;
  const agents = join35(ctx.out, "orchestration", "agents");
...
2. **Queue**: \`${engine} status --out ${ctx.out} --json\` \u2014 every module in the exact order to enrich (unenriched first, hubs first). The enrich phase fans out over \`${join35(ctx.out, "graph.json")}\` + the entries exactly as this queue reports them.
3. **Enrich each module** \u2014 apply \`${join35(agents, "enricher.md")}\` yourself: \`${engine} dossier <slug> --out ${ctx.out}\`, then write 2\u20135 sentences of cited \`[file:line]\` prose into the \`ui:human\` regions of \`${join35(ctx.out, "encyclopedia")}/<slug>.md\`. One module at a time; the hard rule holds here too \u2014 no \`build\` or \`map\` mid-loop.
...
6. **Verify an answer** (high assurance): \`${engine} verify --answer <answer.md> --repo ${ctx.repo}\` writes \`VERIFY.todo.json\` next to the answer. For EVERY pair, apply \`${join35(agents, "refuter.md")}\` yourself (verdict + note), save your rows as \`verdicts.json\`, then \`${engine} verify --apply verdicts.json --answer <answer.md>\` and gate with \`${engine} check --answer <answer.md> --semantic --out ${ctx.out}\`.

With subagents available, prefer the emitted workflows instead: \`orchestrate --out ${ctx.out} --phase <p>\` then \`Workflow({ scriptPath: "${join35(ctx.out, "orchestration", "<p>.workflow.mjs")}" })\` \u2014 one repo-wide \`check\` after the join either way, and no \`build\` or \`map\` while agents are in flight.
`;
}
...
function listPhases(ctx) {
  ...
  const verifyWl = join36(ctx.answer ? dirname10(ctx.answer) : ctx.repo, "VERIFY.todo.json");
  ...
  let verifyReason;
  if (existsSync15(verifyWl)) {
    try {
      const todo = JSON.parse(readFileSync18(verifyWl, "utf8"));
...
function orchestrateRun(ctx, opts = {}) {
  ...
  const orchDir = join36(ctx.out, "orchestration");
  const agentsDir = join36(orchDir, "agents");
  mkdirSync5(join36(orchDir, "out"), { recursive: true });
  ...
  for (const [name2, content] of Object.entries(agentContracts(ctx))) {
    const p = join36(agentsDir, `${name2}.md`);
  ...
  if (!opts.eco) {
    for (const ph of selected) {
      ...
      const p = join36(orchDir, `${ph.name}.workflow.mjs`);
      writeFileSync7(p, phaseWorkflowScript(ph, ctx, BATCH_SIZE));
      written.push(p);
    }
  }
  const rb = join36(orchDir, "RUNBOOK.md");
...
// src/mcp/handlers.ts
import { existsSync as existsSync16, readFileSync as readFileSync19, realpathSync as realpathSync4, statSync as statSync11 } from "fs";
import { isAbsolute as isAbsolute4, join as join37, resolve as resolve8, sep as sep3 } from "path";
...
function resolveOut(args2, repo) {
  ...
  const dotted = join37(repo, ".ultraindex");
  if (existsSync16(dotted)) return dotted;
  const docs = join37(repo, "docs", "ultraindex");
  if (existsSync16(docs)) return docs;
  return dotted;
}
function requireIndex(out2, repo) {
  if (!existsSync16(join37(out2, "manifest.json"))) {
    throw new ToolError(
...
function handleMap(args2, repo, out2) {
  ...
  return { out: out2, module: slug ?? null, index_md: join37(out2, "INDEX.md"), content };
}
...
function handleRead(args2, repo, out2) {
  ...
  const target = isAbsolute4(raw) ? raw : join37(repo, raw);
  let real;
  try {
...
import { existsSync as existsSync17, readdirSync as readdirSync6, readFileSync as readFileSync20, realpathSync as realpathSync5, statSync as statSync12 } from "fs";
import { basename as basename6, dirname as dirname11, join as join38, resolve as resolve9, sep as sep4 } from "path";
...
function resolveSkillRoot(moduleDir) {
  ...
  return candidates.find((dir) => existsSync17(join38(dir, "SKILL.md")));
}
function listResources(moduleDir) {
  const root = resolveSkillRoot(moduleDir);
  if (!root) return [];
  const out2 = [describe(root, "SKILL.md", `${SKILL_NAME}: the skill`)];
  const refDir = join38(root, "references");
  if (!existsSync17(refDir)) return out2;
  for (const file of readdirSync6(refDir).sort()) {
    if (!file.endsWith(".md")) continue;
    out2.push(describe(root, join38("references", file), `${SKILL_NAME} reference: ${basename6(file, ".md")}`));
  }
  return out2;
}
...
function describe(root, rel2, fallbackTitle) {
  const decl = {
    uri: `${URI_SCHEME}${rel2.split(sep4).join("/")}`,
    name: rel2.split(sep4).join("/"),
    title: fallbackTitle,
    mimeType: "text/markdown"
  };
  const summary = firstProse(join38(root, rel2));
...
`;
var COMMANDS = /* @__PURE__ */ new Set(["build", "find", "embed", "neighbors", "symbols", "impact", "delta", "map", "status", "dossier", "ask", "check", "verify", "orchestrate", "grammars", "engine", "mcp"]);
...
function parseArgs(argv) {
  ...
  if (command === "engine") return { command, positional: argv.slice(1), values: {}, bools: /* @__PURE__ */ new Set() };
  ...
  for (let i2 = 1; i2 < argv.length; i2++) {
    ...
    if (arg.startsWith("--")) {
      ...
      if (!VALUE_FLAGS2.has(key)) fail(`unknown flag: --${key} (run --help for the supported options)`);
      let value;
      if (eq !== -1) {
...
function resolveOut2(p, base) {
  ...
  const dotted = join39(base, ".ultraindex");
  if (existsSync18(dotted)) return dotted;
  const docs = join39(base, "docs", "ultraindex");
  if (existsSync18(docs)) return docs;
  return dotted;
}
...
async function cmdBuild(p) {
  ...
  const out2 = p.values.out ? resolve10(p.values.out) : join39(repo, ".ultraindex");
...
function cmdNeighbors(p) {
  ...
  if (kindList) {
    ...
    kinds2 = new Set(kindList);
  }
  const res = runNeighbors(out2, target, depth, kinds2);
  if (!res) fail(`"${target}" is not a module slug or file in the index`);
  if (p.bools.has("json")) {
    process.stdout.write(JSON.stringify(res, null, 2) + "\n");
    return;
  }
  const lines = [`ultraindex: neighbours of ${res.scope} "${res.target}" (depth ${depth}${kinds2 ? `, kind ${[...kinds2].join("/")}` : ""})`, ""];
...
function cmdOrchestrate(p) {
  ...
  const out2 = resolveOut2(p, base);
  const repo = resolveRepoRoot(p, out2);
  const engine = realpathSync6(fileURLToPath4(import.meta.url));
...
`);
  ...
  if (workflows.length) {
    for (const ph of res.phases) {
      const w = workflows.find((x) => x === join39(out2, "orchestration", `${ph.name}.workflow.mjs`));
      if (!w) continue;
    ...
    lines.push(`  next:     follow ${join39(out2, "orchestration", "RUNBOOK.md")} sequentially (the eco path)`);
  }
  process.stderr.write(lines.join("\n") + "\n");
}
async function main() {
