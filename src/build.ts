...
export function runBuild(opts: BuildOptions, builtAt: string): BuildResult {
	...
	const records = new Map<string, FileRecord>(
		scan.files.map((f) => [f.rel, f]),
	);
	// The engine may use one member's comment as its fallback summary. Keep
	// that useful search text, but identify its origin: it is not a description
	// of every file in the module. README/index summaries retain their scope.
	for (const module of modules) {
		const members = module.members.map((rel) => records.get(rel)!);
		const readme = members.find((f) =>
			/^(readme|index)\.(md|mdx)$/i.test(basename(f.rel)),
		);
		if (readme?.summary || readme?.title) continue;
		const source = members.find(
			(f) => f.summary && f.summary === module.summary,
		);
		if (source)
			module.summary = `Excerpt from ${source.rel}: ${module.summary}`;
	}
	...
	writeFileIfChanged(
		paths.symbols,
		renderSymbolsJson(buildSymbolIndex(scan, computeSymbolRefs(scan))),
	);
	...
	writeFileIfChanged(
		paths.index,
		renderIndex(graph, { repoName: basename(opts.repo) || "repo", mermaid }),
	);
	...
	const outRel =
		!isAbsolute(relative(opts.repo, opts.out)) &&
		!relative(opts.repo, opts.out).startsWith("..")
	...
	if (!opts.noCache) {
		...
		for (const f of scan.files)
			files[f.rel] = {
				...
				record: f,
				size: f.size,
				mtimeMs: scan.mtimes.get(f.rel),
			};
		const cacheOut: ExtractionCache = {
			schemaVersion: SCHEMA_VERSION,
