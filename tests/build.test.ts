...
function build(dir: string, mermaid = true) {
	return runBuild({ repo: REPO, out: dir, mermaid, json: false }, FIXED_TIME);
}

describe("runBuild", () => {
	it("attributes a single-file excerpt instead of presenting it as a whole-module explanation", () => {
		const repo = mkdtempSync(join(tmpdir(), "ui-summary-"));
		try {
			...
			writeFileSync(
				join(repo, "src/decoy.mjs"),
				"// This stub does not implement retry scheduling.\nexport const stub = 0;\n",
			);
			writeFileSync(
				join(repo, "src/retry.mjs"),
				"export const retry = () => 100;\n",
			);
			const dir = join(repo, ".ultraindex");
			const result = runBuild(
				{ repo, out: dir, mermaid: false, json: false },
				FIXED_TIME,
			);
			const summary = result.graph.modules.find(
				(m) => m.path === "src",
			)!.summary;
			expect(summary).toBe(
				"Excerpt from src/decoy.mjs: This stub does not implement retry scheduling.",
			);
			expect(readFileSync(join(dir, "encyclopedia/src.md"), "utf8")).toContain(
				summary,
			);
			expect(result.graph.modules.find((m) => m.slug === "root")!.summary).toBe(
				"A documented library.",
			);
		} finally {
	...
	it("surfaces the --max-files cap instead of silently truncating", () => {
		const dir = out();
		const res = runBuild(
			{ repo: REPO, out: dir, maxFiles: 3, mermaid: false, json: false },
			FIXED_TIME,
		);
	...
	it("is byte-identical across two builds of an unchanged repo (graph + entries)", () => {
		...
		for (const f of [
			...
			"graph.mmd",
			"encyclopedia/src.md",
			"encyclopedia/root.md",
		]) {
			expect(readFileSync(join(a, f), "utf8")).toBe(
...
describe("find + neighbors after build", () => {
	it("find returns the right module and the exact files to open", () => {
		...
		expect(
			results.every((r) => r.matched.length > 0 || r.via !== undefined),
		).toBe(true);
	});
